...
def train(rank, num_gpus):
    ...
    # init model
    se_model = model.SE().to(device)
    optimizer = torch.optim.AdamW(se_model.parameters(), learning_rate)
    hamming_window = torch.hamming_window(fft_size, device=device)
    ...
    for epoch in range(saved_epoch + 1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            noisy_mag, noisy_pha = sp.cpx_to_mp(sp.stft(noisy, hamming_window, 0.5, False))
            noisy_log = torch.log1p(noisy_mag)

            clean_log_pred = se_model(noisy_log)

            clean_mag_pred = torch.expm1(clean_log_pred)
            clean_pred = sp.istft(sp.mp_to_cpx(clean_mag_pred, noisy_pha), hamming_window, 0.5, False)

            clean_mag = torch.abs(sp.stft(clean, sin_window, 0.5, False))
            clean_pred_mag = torch.abs(sp.stft(clean_pred, sin_window, 0.5, False))
            loss = sp.ga_lsd2_loss(clean_mag, clean_pred_mag, eps, batch_size)
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(test_loader):
                ...
                if clean.shape[0] != 0:
                    ...
                    noisy_mag, noisy_pha = sp.cpx_to_mp(sp.stft(noisy_pad, hamming_window, 0.5, False))
                    noisy_log = torch.log1p(noisy_mag)

                    clean_log_pred = se_model(noisy_log)

                    clean_mag_pred = torch.expm1(clean_log_pred)
                    clean_pred = sp.istft(sp.mp_to_cpx(clean_mag_pred, noisy_pha), hamming_window, 0.5, False)[:, fft_size:fft_size + noisy.shape[1]]
