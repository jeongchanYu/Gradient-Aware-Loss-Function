...
def train(rank, num_gpus):
    ...
    for epoch in range(saved_epoch + 1, epochs + 1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            clean_mag, clean_pha = sp.cpx_to_mp(sp.stft(clean, sin_window, 0.5, True))
            noisy_mag, noisy_pha = sp.cpx_to_mp(sp.stft(noisy, sin_window, 0.5, True))

            clean_mag_pred = se_model(noisy_mag)

            loss = sp.ga_lsdp_loss(clean_mag, clean_mag_pred, 1, eps, batch_size)
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(test_loader):
                ...
                if clean.shape[0] != 0:
                    ...
                    noisy_mag, noisy_pha = sp.cpx_to_mp(sp.stft(noisy_pad, sin_window, 0.5, True))

                    clean_mag_pred = se_model(noisy_mag)

                    clean_pred = sp.istft(sp.mp_to_cpx(clean_mag_pred, noisy_pha), sin_window, 0.5, True)[:, fft_size:fft_size + noisy.shape[1]]
